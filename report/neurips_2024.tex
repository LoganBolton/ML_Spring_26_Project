...
detection problem: a detector outputs person boxes, and the count
is the number of boxes retained after a confidence threshold. We
compare three models on the same validation split: a
COCO-pretrained YOLO26n (generic zero-shot baseline), SAM\,3 itself
used directly as a zero-shot open-vocabulary counter, and a
YOLO26m fine-tuned on our curated data. A large random
hyperparameter search (195 successful trials across eight sweep
rounds) selects the final model by validation count MSE. The
fine-tuned YOLO26m reaches a per-image count MAE of 1.27 and MSE of
2.79, compared to MAE 3.10 / MSE 15.01 for pretrained YOLO26n and
MAE 5.00 / MSE 33.09 for SAM\,3 zero-shot --- roughly a
5\,$\times$ reduction in squared error over the next-best model.
The main takeaway is that a small amount of in-domain labelled
data, bootstrapped with an open-vocabulary model for labelling, is
enough to beat both a generic pretrained detector and the
open-vocabulary foundation detector itself on this scene.
\end{abstract}
...
\subsection{Third Model Compared: SAM\,3 (zero-shot)}
Because SAM\,3 also produces the pre-annotation labels, it is
natural to ask how well SAM\,3 performs \emph{directly} as a
pedestrian counter --- no fine-tuning, no YOLO, just run SAM\,3
with the text prompt \texttt{"person"} on every validation frame
and count the returned boxes. This gives us a three-way
comparison: a generic pretrained detector (YOLO26n), an
open-vocabulary foundation detector (SAM\,3), and our fine-tuned
specialist (YOLO26m). SAM\,3 is evaluated at its authored
confidence threshold of $0.25$ (the same threshold used for
pre-labelling in \texttt{sam3/annotate.py}); since it is not
trained on our data, no other knobs are tuned.
...
All three models --- pretrained YOLO26n, zero-shot SAM\,3, and
fine-tuned YOLO26m --- are evaluated on the same validation split
using the same preprocessing. For both YOLO models, the
confidence threshold is tuned on the validation set by
grid-search to minimise count MSE. SAM\,3 uses its authored
threshold of $0.25$. Per-image count errors at each model's
selected threshold are reported in Section~\ref{sec:quant}.
...
meaningful spread. Figure~\ref{fig:sweep_progression} visualises
the progression: per-round spread narrows as the search space
tightens, and the running best-so-far curve across all 195
trials decreases from an early-round best of 7.06 to a final
best of 2.79 --- well below the pretrained-baseline reference
line at 15.01.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/sweep_progression.png}
    \caption{Hyperparameter sweep progression. (a) Each dot is
    one successful trial; colour encodes YOLO backbone
    (\texttt{yolo26n/s/m}); the black diamond line tracks the
    best count MSE achieved in each round. Early rounds
    (\texttt{v1}--\texttt{v4}) contain many catastrophic
    \texttt{yolo26n} trials; later rounds concentrate on
    \texttt{yolo26m}. (b) Trials plotted in chronological order,
    with the running best-so-far curve in red and the pretrained
    YOLO26n baseline (MSE 15.01) as a dashed reference.}
...
\label{sec:quant}
Table~\ref{tab:main_results} reports the three-way comparison on
the validation split at each model's chosen confidence threshold.
Fine-tuning reduces count MSE by roughly a factor of five over the
pretrained YOLO baseline and more than a factor of ten over
zero-shot SAM\,3. The fine-tuned model is also nearly unbiased
($-0.07$ people/frame), while pretrained YOLO26n systematically
under-counts ($-1.63$) and SAM\,3 systematically over-counts
($+5.00$), hallucinating people in busy backgrounds (storefront
mannequins, reflections, printed advertising).

\begin{table}[t]
    \centering
    \caption{Main validation-set results. Arrows indicate direction
    of improvement. Training and inference times are recorded on
    the same single-GPU setup. Neither baseline (YOLO26n, SAM\,3)
    is trained on our data, so training time is zero.}
    ...
    \begin{tabular}{lccccc}
        ...
        SAM\,3 zero-shot (\texttt{"person"}) & 5.00 & 33.09 & --- & 0\,s   & ~45\,s / 91 imgs \\
...
baseline and roughly half a second per frame for SAM\,3 --- in
absolute terms, both YOLO variants are more than fast enough to
run in real time on a 1\,FPS livestream, while SAM\,3 is
order-of-magnitude slower and would become a bottleneck at
higher frame rates. Figure~\ref{fig:compute_efficiency}
visualises the accuracy-vs-compute trade-off: the fine-tuned
YOLO26m sits in the preferred corner of the plane --- lower
count MAE than either baseline \emph{and} $\sim$15\,$\times$
faster inference than SAM\,3.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/compute_efficiency.png}
    \caption{Compute efficiency on the Toomer's Corner validation
    split. (a) Per-frame inference latency on a single GPU (log
    scale). (b) Accuracy vs.\ compute Pareto plot: count MAE
    (y-axis) against per-frame inference latency (x-axis, log
    scale). The fine-tuned YOLO26m is the only model in the
    low-latency, low-error region.}
    \label{fig:compute_efficiency}
\end{figure} The full $26$-dim sweep
...
\begin{table}[t]
    ...
    \begin{tabular}{lcc}
        ...
        SAM\,3 zero-shot              & --- & $\sim$500\,ms/frame \\
...
In sparse scenes, both models detect the visible pedestrians
...
count MSE rather than mAP. Fine-tuning cut per-image count MAE to 1.27 (fine-tuned YOLO26m)
from 3.10 (pretrained YOLO26n) and 5.00 (SAM\,3 zero-shot), and
count MSE to 2.79 from 15.01 and 33.09 respectively, on the same
held-out validation split. The fine-tuned model is essentially
unbiased; the pretrained YOLO baseline systematically under-counts
and SAM\,3 systematically over-counts. Compute costs are negligible
...
\begin{itemize}[leftmargin=1.5em]
    ...
    evaluation, SAM\,3 zero-shot counting evaluation,
    hyperparameter search-space pruning between sweep rounds,
